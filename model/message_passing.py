...
class VectorMessagePassing(Model):
    """Messages are generated by elementwise product with a vector instead of a product with a matrix, as in SchNet.

    To generate the message from atom j to atom i, the filter vector belonging to the distance between i and j
    is multiplied elementwise with the hidden state of j. The total message to atom i is the sum of all incoming ones.

    :param hparams: hyperparameters, as a tf.contrib.training.HParams object
    """

    def __init__(self, hparams):
        super(VectorMessagePassing, self).__init__(hparams)

    def _forward(self, hidden_states, vector_filters):
        """Forward pass for generating messages using vector filters.

        :param hidden_states: Hidden states of all atoms, shaped [batch_size, num_atoms, hidden_state_dim]
        :param vector_filters: generated convolution filters shaped [batch_size, num_atoms, num_atoms, hidden_state_dim]
        :return: sum of incoming messages to each atom, shaped [batch_size, num_atoms, hidden_state_dim]
        """
